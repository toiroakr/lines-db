...
describe('LinesDB WHERE Conditions', () => {
  ...
  describe('OR conditions (array syntax)', () => {
    it('should find rows with simple OR condition', () => {
      const result = unwrap(db.find('users', [{ name: 'Alice' }, { name: 'Bob' }])) as unknown as User[];
      expect(result.length).toBe(2);
      expect(result.map((u) => u.name).sort()).toEqual(['Alice', 'Bob']);
    });

    it('should find rows with OR condition on different fields', () => {
      const result = unwrap(db.find('users', [{ name: 'Alice' }, { age: 25 }])) as unknown as User[];
      expect(result.length).toBe(2);
      expect(result.map((u) => u.name).sort()).toEqual(['Alice', 'Bob']);
    });

    it('should work with findOne using OR condition', () => {
      const result = unwrap(db.findOne('users', [{ name: 'Bob' }, { name: 'Charlie' }])) as unknown as User | null;
  ...
  describe('AND conditions with OR (nested)', () => {
    ...
    it('should handle nested arrays (OR within OR)', () => {
      ...
      const result = unwrap(db.find('users', [[{ name: 'Alice' }, { name: 'Bob' }], { age: 35 }])) as unknown as User[];
  ...
  describe('Combining OR and function filters', () => {
    it('should handle OR with function filters', () => {
      ...
      const result = unwrap(
        db.find('users', [{ name: 'Alice' }, { age: (age: unknown) => (age as number) > 30 }]),
      ) as unknown as User[];
      expect(result.length).toBe(2);
      expect(result.map((u) => u.name).sort()).toEqual(['Alice', 'Charlie']);
    });
    ...
    it('should handle complex OR with mixed conditions', () => {
      ...
      const result = unwrap(
        db.find('users', [{ name: 'Bob', age: 25 }, { age: (age: unknown) => (age as number) > 30 }]),
      ) as unknown as User[];
      expect(result.length).toBe(2);
  ...
  describe('Update and Delete with conditions', () => {
    it('should update with OR condition', () => {
      unwrap(db.update('users', { age: 100 }, [{ name: 'Alice' }, { name: 'Bob' }]));

      const result = unwrap(db.find('users', { age: 100 })) as unknown as User[];
      expect(result.length).toBe(2);
      expect(result.map((u) => u.name).sort()).toEqual(['Alice', 'Bob']);
    });

    it('should delete with OR condition', () => {
      unwrap(db.delete('users', [{ name: 'Alice' }, { name: 'Bob' }]));

      const remaining = unwrap(db.find('users')) as unknown as User[];
      expect(remaining.length).toBe(1);
      expect(remaining[0].name).toBe('Charlie');
    });

    it('should return an error when using function filter in update', () => {
      const result = db.update('users', { age: 100 }, { age: (age: unknown) => (age as number) > 25 });
      expect(result.ok).toBe(false);
      if (!result.ok) {
        expect(result.error.message).toContain('Function filters are not supported in update operations');
      }
    });

    it('should return an error when using function filter in delete', () => {
      const result = db.delete('users', { age: (age: unknown) => (age as number) < 30 });
      expect(result.ok).toBe(false);
      if (!result.ok) {
        expect(result.error.message).toContain('Function filters are not supported in delete operations');
      }
    });
  });
  ...
  describe('Edge cases', () => {
    it('should handle empty OR array', () => {
      const result = unwrap(db.find('users', [])) as unknown as User[];
      expect(result.length).toBe(0);
    });

    it('should handle single condition in array', () => {
      const result = unwrap(db.find('users', [{ name: 'Alice' }])) as unknown as User[];
      expect(result.length).toBe(1);
