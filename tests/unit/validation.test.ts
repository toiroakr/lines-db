...
import type { ValidationError } from '@toiroakr/lines-db';
...
describe('LinesDB Validation', () => {
  ...
  describe('insert with validation', () => {
    it('should insert valid data', () => {
      ...
      const user = unwrap(db.findOne('users', { id: 10 }));
  ...
  describe('update', () => {
    ...
    it('should allow any update data (validation is skipped)', () => {
      ...
      const result = unwrap(db.update('users', { email: 'newemail@example.com' }, { id: 1 }));
  ...
  describe('delete', () => {
    it('should delete rows', () => {
      const result = unwrap(db.delete('users', { id: 3 }));
      expect(result.changes).toBe(1);

      const user = unwrap(db.findOne('users', { id: 3 }));
    ...
    it('should delete multiple rows with same condition', () => {
      ...
      const result = unwrap(db.delete('users', { age: 99 }));
